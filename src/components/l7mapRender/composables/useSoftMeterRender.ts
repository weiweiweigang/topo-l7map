...
export function processSoftMeterData(
  scene: Scene,
  topologyKey?: string
) {
  // 获取拓扑数据
  const topologyData = !topologyKey
    ? publicTopology.topologySoftMeter
    : publicTopology.topologySoftMeterMult[topologyKey];
  ...
  for (const item of topologyData) {
    ...
    // 名称标注
    if (isShowEquipmentNameOfMap(221) && item.tpName) {
